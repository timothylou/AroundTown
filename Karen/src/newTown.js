...
export default class Town extends Component{
  ...
  constructor(props) {
    ...
    this._onPressRadioButton = this._onPressRadioButton.bind(this);
        ...
        <View style={Style.mapContainer}>
          ...
          >
            <View>
              ...
              <Text style = {{fontSize: 20, color: '#000000'}}>Select a category for your event:</Text>

              <RadioButton
                onPress = {() => this._onPressRadioButton('freeFood')}
                label = "Free food"
                selected = {this.state.selected.freeFood}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('brokenFacility')}
                label = "Broken facility"
                selected = {this.state.selected.brokenFacility}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('recruiting')}
                label = "Recruiting event"
                selected = {this.state.selected.recruiting}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('studyBreak')}
                label = "Study break"
                selected = {this.state.selected.studyBreak}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('movie')}
                label = "Movie screening"
                selected = {this.state.selected.movie}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('busy')}
                label = "Busy/crowded"
                selected = {this.state.selected.busy}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('fireSafety')}
                label = "Fire safety around"
                selected = {this.state.selected.fireSafety}
              />
              <RadioButton
                onPress = {() => this._onPressRadioButton('other')}
                label = "Other"
                selected = {this.state.selected.other}
              />
  ...
  _onPressRadioButton(propertyName) {
    const select = this.state.selected;
    const setter = select[propertyName];
    for (var key in select) {
      select[key] = false;
    }
    select[propertyName] = !setter;
